...
import {
  ...
  TextField, Divider, Skeleton, Tabs, Tab, Tooltip,
} from '@mui/material';
...
const theme = createTheme({
  ...
  typography: { h4: { fontWeight: 700 }, h5: { fontWeight: 600 }, h6: { fontWeight: 600 } },
});
...
export default function AlertsPage() {
  ...
  const [activeTab, setActiveTab]   = useState(0); // 0=All, 1=Open, 2=Acknowledged, 3=Resolved
  ...
  async function updateStatus(alertId, status) {
    ...
    try {
      ...
      if (res.ok) {
        setAlerts(prev =>
          prev.map(a =>
            a.alert?.id === alertId ? { ...a, alert: { ...a.alert, status } } : a
          )
        );
      }
    } catch {
  ...
  const visible = filterStatus
    ? alertedItems.filter(a => a.alert.status === filterStatus)
    : alertedItems;
    ...
    <ThemeProvider theme={theme}>
      <Box sx={{ backgroundColor: 'background.default', minHeight: '100vh' }}>
        ...
        <Container maxWidth="xl" sx={{ py: { xs: 3, md: 4 } }}>
          ...
          {/* ── Formula Info Card ── */}
          <Card elevation={0} sx={{ border: '1px solid', borderColor: 'grey.100', mb: 3, backgroundColor: alpha('#2E7D32', 0.04) }}>
          ...
          <Grid container spacing={3} sx={{ mb: 3 }} alignItems="center">
            <Grid item xs={12} sm={6} md={4}>
              <Card elevation={0} sx={{ border: '1px solid', borderColor: 'grey.100' }}>
                <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                    ...
                    />
                    <Button variant="contained" size="small" onClick={applyLeadTime} sx={{ borderRadius: 2 }}>
                      Apply
                    </Button>
            ...
            {[
              ...
              <Grid item xs={6} sm={3} md={2} key={s.label}>
                <Card elevation={0} sx={{ border: '1px solid', borderColor: 'grey.100', textAlign: 'center' }}>
                  <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                    {loading
                      ? <Skeleton height={40} />
                  ...
                  {loading ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={9} align="center" sx={{ py: 6 }}>
                        <CheckCircleIcon sx={{ fontSize: 40, color: 'grey.300', mb: 1, display: 'block', mx: 'auto' }} />
                        <Typography color="text.secondary" variant="body2">
                          {filterStatus
                            ? `No ${filterStatus} alerts.`
                            : 'No alerts — all products are sufficiently stocked.'}
                        </Typography>
                      </TableCell>
                    </TableRow>
                  ) : (
                    visible.map(item => {
                      ...
                      return (
                        <TableRow key={item.productId}
                          sx={{
                            backgroundColor: item.stockStatus === 'critical' || item.stockStatus === 'out_of_stock'
                              ? alpha('#D32F2F', 0.04)
                          ...
                          <TableCell>
                            <Chip label={stockCfg.label} color={stockCfg.color} size="small"
                              sx={{ fontWeight: 600 }} />
                          </TableCell>
                          <TableCell>
                            <Tooltip title="Avg units transferred per day (last 30 days)" arrow>
                              <Typography variant="body2">
                                {item.velocity > 0 ? item.velocity.toFixed(2) : '—'}
                              </Typography>
                            </Tooltip>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            {alertCfg ? (
                              <Chip label={alertCfg.label} color={alertCfg.color} size="small"
                                icon={alertCfg.icon} sx={{ fontWeight: 600 }} />
                            ) : (
                              ...
                              <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
                                {item.alert.status === 'open' && (
                                  <Button size="small" variant="outlined" color="warning"
                                    ...
                                    sx={{ borderRadius: 2, fontSize: '0.7rem', py: 0.3 }}>
                                    Acknowledge
                                  </Button>
                                )}
